...
3. **`org_id` is trusted, unvalidated, client-suppliable metadata at signup (Task 9's `handle_new_auth_user` trigger reads it straight from `raw_user_meta_data`).** Not exploitable today — this plan's Task 1 (Auth/Landing) only implements login (`signInWithPassword`), never public sign-up, and demo users are provisioned via a service-role script (Task 17), not the public Auth API. But if any future work ever adds public self-service sign-up, `org_id` must be validated server-side (an invite code, an allowlist, or a trusted server action assigning it) before reaching `raw_user_meta_data` — otherwise anyone could sign up claiming any org's id (org ids are publicly listable via the intentionally-open `organization` table) and gain full RLS-scoped access to that org's data.
