...
export default async function MissionDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const { data: donor } = await supabase.from("hospital").select("location").eq("id", mission.donorHospital.id).single();
  const { data: recipient } = await supabase.from("hospital").select("location").eq("id", mission.recipientHospital.id).single();

  const mapMarkers: MapMarker[] = [];
  const donorLngLat = pointToLngLat(donor?.location as any);
  const recipientLngLat = pointToLngLat(recipient?.location as any);
